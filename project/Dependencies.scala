...
object Dependencies {
  ...
  // Named ironModule rather than iron: the bare `iron` artifact takes the val
  // of that name below, and a def and a val cannot share one.
  private def ironModule(a: String) =
    "io.github.iltotore" %% s"iron-$a" % Version.iron

  private def jsoniter(a: String) =
    "com.github.plokhotnyuk.jsoniter-scala" %% s"jsoniter-scala-$a" % Version.jsoniter
  ...
  lazy val http4sClient = http4s("client")
  ...
  // JSON
  lazy val jsoniterCore = jsoniter("core")

  lazy val jsoniterMacros = jsoniter("macros") % Provided

  /** Parses straight into circe's AST, which is what lets this build use
    * jsoniter's parser without giving up an AST for the arbitrary JSON the
    * specification requires to survive a round trip.
    */
  lazy val jsoniterCirce = jsoniter("circe")

  lazy val circeCore    = circe("core")
  lazy val circeGeneric = circe("generic")
  lazy val circeParser  = circe("parser")

  lazy val circeAll = Seq(circeCore, circeParser)

  // Data
  lazy val chimney = "io.scalaland" %% "chimney" % Version.chimney

  lazy val iron           = "io.github.iltotore"    %% "iron" % Version.iron
  lazy val ironCirce      = ironModule("circe")
  lazy val ironJsoniter   = ironModule("jsoniter")
  lazy val ironZioJson    = ironModule("zio-json")
  lazy val ironChimney    = ironModule("chimney")
  lazy val ironDoobie     = ironModule("doobie")
  lazy val ironSkunk      = ironModule("skunk")
  lazy val ironPureconfig = ironModule("pureconfig")
  lazy val ironScalacheck = ironModule("scalacheck") % Test
  ...
  /** The only crypto dependency the published modules use, confined to jose. */
  ...
  lazy val testDeps = Seq(munit, munitScalacheck, munitCatsEffect)
